...
class FullFinetuneRecipeDistributedAnsIntl(FTRecipeInterface):
    ...
    def _loss_step(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        """
        Retain the question tokens, interleave answer tokens with entropy tokens.
        """
        ...
        # Determine answer start and answer lengths from labels
        questions = []     # list of 1D tensors for question tokens per sample
        answers = []       # list of 1D tensors for answer tokens per sample
        question_lengths = []
        answer_lengths = []  # number of answer tokens for each sample
        
        for i in range(batch_size):
            label_i = labels[i]  # shape: [s]
            non_pad_indices = (label_i != -100).nonzero(as_tuple=False).squeeze(1)
            
            answer_start = non_pad_indices[0].item()
            answer_length = non_pad_indices.numel()  # assume contiguous answer tokens
            answer = tokens[i, answer_start: answer_start + answer_length]
            question = tokens[i, :answer_start]
        ...
        discretized = self.discretize_num(torch.max(probs, dim=-1)[0])
        ...
        max_new_len = max(question_lengths) + max(answer_lengths) * 2
        input_samples = torch.full((batch_size, max_new_len), pad_token_id, device=self._device)
        
        for i in range(batch_size):
            q_len = question_lengths[i]
            a_len = answer_lengths[i]
            # Fill question part
            input_samples[i, :q_len] = tokens[i, :q_len]
            # Fill interleaved answer + entropy part
            interleaved = torch.stack(
                (answers[i], discretized[i][q_len : q_len + a_len]), dim=-1
            ).flatten()
            input_samples[i, q_len:q_len + a_len * 2] = interleaved
        ...
        for i in range(batch_size):
            question_positions = torch.arange(0, question_lengths[i], step=1, device=tokens.device)
            answer_positions = torch.arange(question_lengths[i]+1, question_lengths[i]+1 + answer_lengths[i]*2, step=2, device=tokens.device)
            prediction_positions = torch.cat((question_positions, answer_positions))
