...
class FullFinetuneRecipeDistributedAnsIntl(FTRecipeInterface):
    """
    Full finetuning recipe for dense transformer-based LLMs such as Llama2. This recipe supports
    distributed training and can be run on a single node (1 to 8 GPUs).

    This recipe has been modified to implement answer-only interleaved entropy training. The model 
    makes forward passes twice: first to compute entropy values for each token position, and a 
    second time with a modified sequence where only the answer tokens are interleaved with 
    entropy tokens (question tokens remain unchanged). This approach focuses the uncertainty
    learning on the generative part of the model's output.
    ...
    def left_pad(self, seq: torch.Tensor, max_len: int, pad_token: int) -> torch.Tensor:
        """Left pad a sequence to the specified length."""
        num_pad = max_len - seq.size(0)
        if num_pad > 0:
            pad_tensor = torch.full((num_pad,), pad_token, dtype=seq.dtype, device=seq.device)
            return torch.cat([pad_tensor, seq], dim=0)
        return seq
    
    def right_pad(self, seq: torch.Tensor, max_len: int, pad_token: int) -> torch.Tensor:
    ...
    def discretize_entropy(self, entropy) -> torch.Tensor:
    ...
    def discretize_softmax(self, softmax) -> torch.Tensor:
    ...
    def discretize_num(self, softmax) -> torch.Tensor:
    ...
    def generate_attention_mask_and_position_ids(self, tokens: torch.Tensor, pad_token_id: int, max_generated_tokens: int) -> Tuple[torch.Tensor, torch.Tensor]:
        """Generate attention mask and position ids for the input tokens."""
        from torchtune.generation._generation import get_causal_mask_from_padding_mask, get_position_ids_from_padding_mask
        padding_masks = (tokens != pad_token_id)
        # Extend question mask by max_generated_tokens to accommodate for entropy values, answer masks are all true 
        padding_masks = torch.nn.functional.pad(padding_masks, (0, max_generated_tokens), value=True)
        masks = get_causal_mask_from_padding_mask(padding_mask=padding_masks, target_seq_len=None)
    ...
    def _find_subsequence(self, haystack: torch.Tensor, needle: torch.Tensor) -> int:
        """Find first occurrence of needle tensor within 1D haystack tensor.
           Returns the index in haystack *after* the needle ends, or -1 if not found."""
        n_len = needle.shape[0]
        h_len = haystack.shape[0]
        if n_len == 0 or h_len == 0 or n_len > h_len:
            return -1
        # Ensure needle is on the same device
        needle = needle.to(haystack.device)
        # Create rolling windows of the haystack
        rolling_window = haystack.unfold(0, n_len, 1)
        # Check for equality in each window
        matches = torch.all(rolling_window == needle, dim=1)
        match_indices = torch.nonzero(matches, as_tuple=True)[0]
        if len(match_indices) == 0:
            return -1
        # Return the index *after* the first match
        return (match_indices[0] + n_len).item()

    def _loss_step(self, batch: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Performs a training step with reasoning-only interleaving.
        1. Find Q, R, A sections (R starts after marker, A determined by labels).
        2. First forward pass on original input for token probabilities.
        3. Create new input: Q + interleaved(R) + A.
        4. Second forward pass on new input.
        5. Extract hidden states for A positions from the second pass.
        6. Compute loss only on A positions.
        Returns:
            torch.Tensor: Mean loss over the answer tokens.
            torch.Tensor: Number of active (answer) tokens used in loss calculation.
        """
        ...
        marker_sequence = torch.tensor([36287, 220, 16, 25], device=self._device) 
        ...
        for i in range(batch_size):
            label_i = labels[i]
            token_i = tokens[i]
            non_pad_indices = (label_i != ignore_index).nonzero(as_tuple=False).squeeze(1)

            ans_start = non_pad_indices[0].item()
            ans_len = non_pad_indices.numel()

            # Find end of question / start of reasoning using marker
            search_haystack = token_i[:ans_start]
            print(f"token {i}: {token_i}")
            print(f"label {i}: {label_i}")
            reas_start = self._find_subsequence(search_haystack, marker_sequence)
            print(f"reas_start: {reas_start}")

            if reas_start == -1: # Marker not found
                print(f"Marker not found for sample {i}")
                print(token_i)
                reas_start = ans_start # Treat everything before answer as question
                q_len = ans_start
                r_len = 0
            else:
                q_len = reas_start
                r_len = ans_start - reas_start

            q_lens.append(q_len)
            r_lens.append(r_len)
            a_lens.append(ans_len)
            reasoning_starts.append(reas_start)
            answer_starts.append(ans_start)
            print(f"q_lens: {q_lens}")
            print(f"r_lens: {r_lens}")
            print(f"a_lens: {a_lens}")
            print(f"reasoning_starts: {reasoning_starts}")
            print(f"answer_starts: {answer_starts}")

        num_layers = len(self._model.layers)
        self._model.output_hidden_states = [num_layers]
        
        with torch.no_grad():
            with self.activations_handling_ctx:
                logits_orig = self._model(
                    tokens=tokens,
                    mask=None,
        ...
        # Compute tokenwise probabilities and entropy.
        probs = torch.nn.functional.softmax(logits_orig, dim=-1)
        discretized = self.discretize_softmax(torch.max(probs, dim=-1)[0])
        ...
        # interleave answer with corresponding entropies
        max_new_len = max(q_lens) + max(r_lens)*2 + max(a_lens) 
        print(f"max_new_len: {max_new_len}")
        input_samples = torch.full((batch_size, max_new_len), pad_token_id, device=self._device)
        
        for i in range(batch_size):
            q_len, r_len, a_len = q_lens[i], r_lens[i], a_lens[i]
            reas_start, ans_start = reasoning_starts[i], answer_starts[i]

            input_samples[i, :q_len] = tokens[i, :q_len]
            reasoning_tokens = tokens[i, reas_start:ans_start]
            entropy_tokens = discretized[i, reas_start:ans_start]
            interleaved_reasoning = torch.stack((reasoning_tokens, entropy_tokens), dim=-1).flatten()
            input_samples[i, q_len : q_len + r_len * 2] = interleaved_reasoning
            input_samples[i, q_len + r_len * 2 : q_len + r_len * 2 + a_len] = tokens[i, ans_start : ans_start + a_len]
            print(f"input_samples: {input_samples[i]}")

        with self.activations_handling_ctx:
            logits_new = self._model(
                tokens=input_samples,
                mask=None,
                encoder_input=batch.get("encoder_input"),
                encoder_mask=batch.get("encoder_mask"),
                input_pos=None,
            )
            logits_new = logits_new[0]  # [b, new_seq_len, vocab_size]
        
        # Pre-allocate extracted_logits
        extracted_logits = torch.zeros(batch_size, seq_len, logits_new.size(-1), dtype=logits_new.dtype, device=logits_new.device)
        
        for i in range(batch_size):
            question_positions = torch.arange(0, q_lens[i], step=1, device=tokens.device)
            reasoning_positions = torch.arange(q_lens[i]+1, q_lens[i]+1+r_lens[i]*2, step=2, device=tokens.device)
            answer_positions = torch.arange(q_lens[i]+r_lens[i]*2, q_lens[i]+r_lens[i]*2+a_lens[i], step=1, device=tokens.device)
            prediction_positions = torch.cat((question_positions, reasoning_positions, answer_positions))
            print(i)
            print(f"prediction_positions: {prediction_positions}")
            print(f"len(prediction_positions): {len(prediction_positions)}")
            extracted_logits[i, :len(prediction_positions), :] = logits_new[i, prediction_positions, :]
            print(f"extracted_logits: {extracted_logits[i]}")
        # Unembedding and loss calculation
        output_logits = self._model.unembed(extracted_logits)
        ...
        if not isinstance(output_logits, list):
            labels = labels.reshape(-1)
            output_logits = output_logits.reshape(-1, output_logits.size(-1))
            
        # Compute loss
        loss = self._loss_fn(output_logits, labels)
...
def recipe_main(cfg: DictConfig) -> None:
    ...
    """
    config.log_config(recipe_name="FullFinetuneRecipeDistributedAnsIntl", cfg=cfg)
    recipe = FullFinetuneRecipeDistributedAnsIntl(cfg=cfg)
