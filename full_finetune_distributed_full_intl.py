...
class FullFinetuneRecipeDistributedIntl(FTRecipeInterface):
    """
    Full finetuning recipe for dense transformer-based LLMs such as Llama2. This recipe supports
    distributed training and can be run on a single node (1 to 8 GPUs).

    This recipe has been modified to implement interleaved entropy training, where each token
    is interleaved with a token representing its entropy. The model makes forward passes twice:
    once to compute entropy values for each token position, and a second time with the interleaved
    sequence of original tokens and entropy tokens. This helps the model learn to predict uncertainty
    ...
    def generate_attention_mask_and_position_ids(self, tokens: torch.Tensor, pad_token_id: int, max_generated_tokens: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        from torchtune.generation._generation_original import get_causal_mask_from_padding_mask, get_position_ids_from_padding_mask
    ...
    def _loss_step(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        """
        Interleave full sequence with entropy tokens.
        """
        ...
        # Build the new (extended) sequence by interleaving each original token with its entropy token.
        extended_tokens = torch.stack((tokens, discretized), dim=-1) # [b, seq_len, 2]
        extended_tokens = extended_tokens.flatten(start_dim=1)  # [b, 2*seq_len]
        ...
        # Extract model predictions only at positions corresponding to the original tokens.
        gt_positions = torch.arange(1, 2 * seq_len, step=2, device=tokens.device)
        extracted_logits = logits_new[:, gt_positions, :]  # [b, seq_len, vocab_size]
        # Unembedding and loss calculation remain unchanged.
...
def recipe_main(cfg: DictConfig) -> None:
    ...
    config.log_config(recipe_name="FullFinetuneRecipeDistributedIntl", cfg=cfg)

    recipe = FullFinetuneRecipeDistributedIntl(cfg=cfg)
