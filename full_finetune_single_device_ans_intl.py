...
class FullFinetuneRecipeSingleDevice(FTRecipeInterface):
    ...
    def discretize_softmax(self, softmax) -> torch.Tensor:
        ...
        #int_softmax = torch.randint(0, 11, softmax.shape, device=softmax.device, dtype=torch.int)
        #random_mask = torch.rand(softmax.shape, device=softmax.device) < 0.1
        #random_values = torch.randint(0, 11, softmax.shape, device=softmax.device, dtype=torch.int)
        #int_softmax = torch.where(random_mask, random_values, torch.full_like(random_values, 10))
        print(int_softmax)
    ...
    def _loss_step(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        """
        Retrain the question tokens, interleave answer tokens with entropy tokens.
        """
